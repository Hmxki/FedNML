...
class LA_KD_s(nn.Module):
    ...
    def forward(self, x, target, w_kd):
        # Logit Adjustment
...
class LA_KD(nn.Module):
    def __init__(self, cls_num_list, num, active_class_list, tau=1, weight=None):
    ...
    def forward(self, x, target, soft_target, w_kd):
        ...
        #kd_loss = kd_loss[:, self.active_class_list]
        ...
        #cls_loss = cls_loss[:, self.active_class_list]
...
class LocalUpdate(object):
    ...
    def correct_labels_with_Gmm(self, glob_model, w, global_round, clean_interval=5,weight_kd=0.2):
        #clean_interval=5
        ...
        # 方法 A: 一致性得分
        if global_round==10:
            consistency = (all_labels[:,self.active_class_list] * all_probs[:,self.active_class_list] + (1 - all_labels[:,self.active_class_list]) * (1 - all_probs[:,self.active_class_list])).mean(dim=1)
        ...
        # 方法 B: BCE损失
        if global_round == 10:
            loss = F.binary_cross_entropy(all_probs[:,self.active_class_list], all_labels[:,self.active_class_list], reduction='none').mean(dim=1)
        else:
        ...
        for i in noisy_item_ids:
            ...
            #sum_+=arr.sum()
            self.dataset_ALR[i][self.active_class_list] = arr[self.active_class_list]

        for i in range(len(all_indices)):
            arr = all_probs[i]
            arr = np.where(arr > 0.9, 1, np.where(arr < 0.1, 0, arr))
            # self.dataset_ALR[all_indices[i].item()][torch.where(self.label_mask.to('cpu') == 1)] = arr[torch.where(
            #     self.label_mask.to('cpu') == 1)] * 0.4 + 0.6* self.dataset_ALR[all_indices[i].item()][torch.where(
            #     self.label_mask.to('cpu') == 1)]

            self.dataset_ALR[all_indices[i].item()][torch.where(self.label_mask.to('cpu') == 1)] = arr[torch.where(self.label_mask.to('cpu') == 1)]*0.2+0.8*self.dataset_ALR[all_indices[i].item()][torch.where(self.label_mask.to('cpu') == 1)]

        #print(f"[Client {self.client_id}] Corrected {sum_} noisy labels.")
        self.class_num_list = list(np.sum(self.dataset_ALR[all_indices],axis=0)*weight_kd+np.array(self.class_num_list)*(1-weight_kd))
    ...
    def update_weights(self, model, global_round,weight_kd):
        ...
        self.correct_labels_with_Gmm(glob_model=deepcopy(model), w=[alpha, beta],
                                          global_round=global_round,weight_kd=weight_kd)
        ...
        if global_round<10:
            ...
            # criterion = LA_KD_s(cls_num_list=self.class_num_list, num=len(self.trainloader.dataset),
            ...
            #self.class_num_list = list(np.sum(self.dataset_ALR, axis=0))
            criterion = LA_KD(cls_num_list=self.class_num_list, num=len(self.trainloader.dataset),
                              active_class_list=self.active_class_list)
            for iter in range(self.args.local_ep):
                batch_loss = []
                #all_indices=[]
