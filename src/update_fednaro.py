...
class LA_KD(nn.Module):
    ...
    def forward(self, x, target, soft_target, w_kd):
        ...
        cls_loss = cls_loss[:,self.active_class_list_client].mean()
...
class LocalUpdate(object):
    ...
    def update_weights(self, model, global_round, weight_kd):
        ...
        if global_round<10:
            ...
            alpha, beta = self.sigmoid_weights_clipped(global_round, max_epoch=200, k=0.1, min_val=0.1, max_val=0.9)
            ...
            for iter in range(self.args.local_ep):
                batch_loss = []
                for batch_idx, (images_w, image_s, labels, items) in enumerate(self.trainloader):
                    images_w = images_w.to(self.device)
                    labels = labels.to(self.device)
                    logits1 = model(images_w)
                    ...
                    #labels[:, torch.where(self.label_mask.to('cpu') == 1)[0]] = logist_glo_sig[:, torch.where(self.label_mask.to('cpu') == 1)[0]]
                    loss = criterion(logits1, labels, logist_glo_sig, weight_kd)
