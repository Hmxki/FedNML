...
from update_others import LocalUpdate, test_inference
from models import MLP, CNNMnist, CNNFashion_Mnist, CNNCifar
from utils import get_dataset, average_weights, exp_details, FedAvg_tao, FedAvg_proto, Logger
...
if __name__ == '__main__':
    ...
    log_file = os.path.join("../output_print/brazilian", "16missing-fedmlp.txt")
    ...
    dict_len = [len(user_groups[idx]) for idx in idxs_users]
    local_models = []
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for idx in idxs_users:
            ...
            if args.exp == 'FedMLP':
                if epoch < args.rounds_FedMLP_stage1 - 1:
                    w_local, loss_local, negetive_class_list_client, active_class_list_client = local_model.train_FedMLP(
                        epoch, tao, Prototype, net=copy.deepcopy(global_model).to(device))
                else:
                    w_local, loss_local, negetive_class_list_client, active_class_list_client, tao_local, Prototype_local = local_model.train_FedMLP(
                        epoch, tao, Prototype, net=copy.deepcopy(global_model).to(device))

            if args.exp == 'FedMLP' and epoch >= args.rounds_FedMLP_stage1 - 1:
                taos.append(copy.deepcopy(tao_local))
                Prototypes.append(copy.deepcopy(Prototype_local))
            if epoch == 0 and args.exp != 'RoFL':
            ...
            local_losses.append(copy.deepcopy(loss_local))
